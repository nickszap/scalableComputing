...
__global__ void s2g_gpu_scatter_kernel(int *in, int *out, int len) {
  //@@ INSERT CODE HERE
	...
	if (inIdx<len){ //avoid indexing outside array bounds
...
__global__ void s2g_gpu_gather_kernel(int *in, int *out, int len) {
  ...
  if (outIdx<len){ //avoid indexing outside array bounds
...
static void s2g_gpu_scatter(int *in, int *out, int len) {
  ...
  const unsigned int BLOCK_SIZE = 32; // Use 1D, 16 thread blocks
	
  dim3 gridDim((len-1)/BLOCK_SIZE + 1, 1, 1);
  dim3 blockDim(BLOCK_SIZE, 1, 1);

  // Invoke CUDA kernel -----------------------------------------------------
  s2g_gpu_scatter_kernel <<< gridDim, blockDim >>> (in, out, len);
}
...
static void s2g_gpu_gather(int *in, int *out, int len) {
  ...
  // Invoke CUDA kernel -----------------------------------------------------
  s2g_gpu_gather_kernel <<< gridDim, blockDim >>> (in, out, len);;
}
